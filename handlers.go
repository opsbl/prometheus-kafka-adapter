...
func receiveV2Handler(producer *kafka.Producer, serializer Serializer) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		metricsPerTopic, err := processWriteRequestWithSerialize(&req, SerializeToEasyOps)
		if err != nil {
			c.AbortWithStatus(http.StatusInternalServerError)
			logrus.WithError(err).Error("couldn't process write request")
			return
